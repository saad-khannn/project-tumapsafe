...
def getDateTime(service, user_id, msg_id, alert):
    try:
        message = service.users().messages().get(
            userId=user_id, id=msg_id, format='minimal').execute()
    ...
    for thread in threads:
        ...
        if check == None:
            ...
            # If the TUalert is an all clear message
                ...
                alert.description = 'All clear in the area of' + alert.location + \
                    '. Suspect in custody. You may resume normal operations.'
        ...
        # Increment the amount of alerts
        totalAlerts += 1
